...
<template>
  <div class="search-layout">
    ...
    <el-input
      ...
      <el-button slot="append" @click="appendClick">{{this.appendBtnTitle}}</el-button>
    </el-input>
  </div>
</template>
...
<script>
export default {
  name: 'ScanContentComponentAndAppendV2',
  props: {
  ...
  methods: {
    searchAction () {
      ...
      this.$emit('input', this.searchContent)
    },
    processIsNumber () {
    ...
    appendClick () {
      this.searchContent = this.searchContent.trim()
      this.$emit('on-append-click', this.searchContent)
    },
    // form-create
    // 表单禁用事件
    onDisabled (disabled) {
